...
router.get("/", (req, res) => {
  db.all(
    "SELECT * FROM materials WHERE in_stock = 1 ORDER BY name",
    (err, rows) => {
...
router.get("/price/estimate", (req, res) => {
  ...
  if (!material_id || !weight_grams) {
    return res
      .status(400)
      .json({ error: "material_id and weight_grams are required" });
  }
  ...
  db.get(
    ...
    (err, material) => {
      ...
      const estimated_price = (
        material.price_per_gram * parseFloat(weight_grams)
      ).toFixed(2);
      res.json({
...
router.post("/", auth, adminOnly, (req, res) => {
  ...
  if (!name || !price_per_gram) {
    return res
      .status(400)
      .json({ error: "Name and price_per_gram are required" });
  }
  ...
  db.run(
    ...
    function (err) {
      ...
      res
        .status(201)
        .json({ id: this.lastID, message: "Material added successfully" });
    },
  );
});

router.put("/:id", auth, adminOnly, (req, res) => {
  const { name, description, color, price_per_gram, properties, in_stock } =
    req.body;
